...
async def check_subscriptions_expiry(bot: AsyncTeleBot, user_id=0):
    ...
    # Получаем всех пользователей с подпиской
        ...
        if not user_data or (not subscription_end and subscription_end is not None):
            logger.info(f"subscription_end = {user_data.get("subscription_end")} - {type(user_data.get("subscription_end"))}")
